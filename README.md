...
- ∇·g > 0: 소스 영역 (outflow, 질량/원천이 있는 영역)
- ∇·g < 0: 싱크 영역 (inflow, 흡수/소멸이 있는 영역)
- ∇·g = 0: 라플라시안 구조 (locally solenoidal, 보존 영역)

**참고**: 발산은 플럭스/소스·싱크/포아송 구조(라플라시안) 관점이며, 일반 동역학에서 안정성(고정점 안정/불안정)을 바로 판정하는 스칼라가 아닙니다. 안정성 판단은 보통 Jacobian/Hessian(또는 Lyapunov)로 별도 수행합니다.
...
발산이 큰 영역(∇·g > 0)은 소스 영역으로, 플럭스가 외부로 나가는 영역입니다. 순수 퍼텐셜 필드의 경우 Poisson 방정식 ∇²V = -∇·g로 연결됩니다.

### 회전 영역 (비보존력)

회전이 큰 영역(∇×g ≠ 0)은 비보존력이 작용하는 왜곡 영역입니다. 순수 퍼텐셜 필드(g = -∇V)는 이론상 curl = 0이므로, curl ≠ 0이면 비퍼텐셜 성분(예: 마그네틱 필드, 난류) 탐지 가능합니다.

### 발산 음수 영역 (싱크/흡수)

발산이 음수인 영역(∇·g < 0)은 싱크 영역으로, 플럭스가 내부로 흡수되는 영역입니다.
